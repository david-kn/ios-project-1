...
void showHelp() {
    printf("Napoveda programu\n");
    printf("Autor: David Konar (xkonar07@stud.fit.vutbr.cz\n\n");
    printf("Uziti: ./radauth -c <konfiguracni soubor>\n\n");
    printf("Parametry:\t-c <config filename>\tpovinny parametr\n");
}
...
int parseConfigFile(ptrSettings settings) {
    ...
    try {
       ...
       while(!infile.eof()) {
         ...
         //infile.close();  // we don't know where the problem appeared so rather don't close it
         return E_CONF;
    }
    infile.close();
    return E_OK;
    ...
    bool NASid = false;
     ...
     for(i = 20; i < myPacket->length;) {
                ...
                q = i;
                        ...
                        if((unsigned char)msg[i] == 1) {    // User-Name
                            type = 1;
                            name = true;
                        }
                        else if((unsigned char)msg[i] == 2) { // User-Password
                            type = 2;
                            password = true;
                        }
                         else if((unsigned char)msg[i] == 32) { // NAS-Identifier
                            type = 32;
                            NASid = true;
                        }
                        else {      // ignore
                ...
                i++;
                       ...
                       if(type == 2) {
                           //printf("Delka hesla: %d\n",msg[i]-2);
                            if((unsigned char)msg[i]-2 < 16) {  // to short user-password (minimum is 18 chars)
                                globalCondition = false;
                            }
                        }
                        ...
                        if(type == 32) {
                            //printf("Delka ID: %d\n",msg[i]-2);
                            if((unsigned char)msg[i]-2 < 3) { // to short NAS-Identifier (minimum is 3 chars)
                                globalCondition = false;
                            }
                        }
...
int processIncome(char* buffer, tSettings *settings, bool &verified, bool &discard, tPacket *myPacket) {
    ...
    /**********************************/
        ...
        if(verified == true) {     // user-name not found, skip it!
            ...
            if(password.length() != myPacket->passLength) {
            ...
            } else {    // password have the same length
                ...
                cnt = 0;
                for(; cnt < 16; cnt++) {   // "cnt" is set to "0" for the first time
			  ...
			  // hash the whole string  => output is in 'MD5result'
                    MD5((unsigned char*) toHash.c_str(), toHashLength, MD5result);

                    zero = 0;
                    for(; cnt < backward + 16; cnt++) {   // "cnt" is set to "0" for the first time
...
/**********************************/
    ...
    // We'll get amount of interfaces in the PC
    interfaces = ifconf.ifc_len / sizeof(ifreq[0]);

    val = 0;

    // cycle through all of the interfaces
    for (i = 0; i < interfaces;i++) {


        struct sockaddr_in *address = (struct sockaddr_in *) &ifreq[i].ifr_addr;

        // Converts the binary IP address into a string so we can compare it
        ...
        for ( it = (settings->interface).begin(); it != (settings->interface).end(); ++it) {
          if(it->compare(ifreq[i].ifr_name) == 0) {
			    ...
			    servAddr.sin_addr.s_addr = inet_addr(ip);		// specific address (int)
                      ...
                      if(rc < 0) {
                        return E_BIND;
                          ...
                          if ((flags=fcntl(mysocket[i],F_GETFL,0))< 0) { // check out current flags of the socket
    ...
    // save it to global interface so program will know huch manu sockets were opened!
    howmuch = settings->numInterfaces;

      // at least 1 interface wasn't opened - WRONG definition in config file - exit program with error
	if(val != settings->numInterfaces) {
      	return E_INT;
      }


    // init buffer to zeros
    ...
    while (true) {
        ...
        for (i = 0; i <= settings->numInterfaces; ++i) {
             ...
             if(settings->numInterfaces == 1) {  // IMPORTANT!! If only 1 interface is set in config file, then break it after just 1 iteration!
                break;
             }
        }
            ...
            if (FD_ISSET(mysocket[i], &readset)) {
                // if this socket has some data to read
                        ...
                        if((n = recvfrom(mysocket[i], msg, MAX_MSG, 0, (struct sockaddr *) &cliAddr, &cliLen)) >= 0) {
                                ...
                                if(n == myPacket.length) { // length is same - OK
                                    if(discard == false) {  // if the packet is valid then send response (accept or reject) (otherwise retain silent)
                                        plen = createResponse(verified, msg, sendToClient, settings, &myPacket);
                                        sendto(mysocket[i],sendToClient,plen,flags,(struct sockaddr *)&cliAddr,cliLen);
                                        delete[] sendToClient;
                                    }
                                } else {     // length is different! discard - do nothing...
                                    ;
                                }
                        }
...
// tries to clean all the memory & close sockets before the program will exit
void clean_all (int sig) {
    int i = 0;
    if(allocated == true) {
        // try to close all opened sockets
        for(i = 0; i < howmuch; i++) {
            if(close(mysocket[i])== 0) {
            	// if failed... so what, we've been killed by signal anyway
                    ;
            }
        }
        // free the memory
...
int main(int argc, char** argv)
{
    ...
    if(((e = processArg(argc, argv, &settings)) == E_HELP)) { // show help
        showHelp();
        return EXIT_SUCCESS;

    }   else if (((e = processArg(argc, argv, &settings)) != E_OK)) { // bad arguments
        showError(e);
        return EXIT_FAILURE;

    } else {    // everything is fine... continue
        ;
    }
